...
const Menu = memo(({onChange}: { onChange: (data: any) => void }) => {
    const {data, isLoading: isCalculateLoading} = useQuery("calculate", () => calculateApi());
    const {data: variableMap, isLoading: isVariableLoading} = useFormulaVariableMap();
    const [active, setActive] = useState(0)

    const itemNames = data?.map((item: any) => variableMap[item.name]?.name || item.name) || [];
    const loading = isCalculateLoading || isVariableLoading;
